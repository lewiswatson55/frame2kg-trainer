...
This scaffold includes a training entrypoint and example configs for Qwen (2.5/3/3.5-compatible backend), Gemma 4 E2B/E4B instruct, and SmolVLM2 with LoRA. You must have access to the referenced model checkpoints.
...
```bash
python scripts/train.py --config configs/gemma4vl/gemma4_e2b_it_lora.yaml
```
...
Common keys:
- `backend`: which implementation to use (e.g., `qwen25_vl`, `qwen3_vl`, `qwen35_vl`, `gemma4_vl`, or `smolvlm2`).
...
- `disable_thinking`: when true (default), disables reasoning tokens in model families that expose them through chat templating.
...
- `frame2kg_train/backends/`
  ...
  - `gemma4_vl.py`: Gemma 4 E2B/E4B instruct backend (AutoProcessor + AutoModelForImageTextToText), with optional 4-bit quant, optional LoRA, and Gemma-specific response parsing.
...
- `frame2kg_train/data/`
  ...
  - `collators.py`: Qwen, Gemma, and Smol chat formatting (`QwenVLDataCollator`, `Gemma4DataCollator`, `SmolVLMDataCollator`) that build supervised chat samples and mask labels appropriately.
...
- `configs/<model-type>/*.yaml`: Example configs grouped by model family (`qwen25vl`, `qwen3vl`, `qwen35vl`, `gemma4vl`, `smolvlm2`).
...
- A fully wired Gemma 4 E2B/E4B instruct backend with optional 4‑bit loading and LoRA adapters (via PEFT).
...
- A Gemma 4-compatible collator that uses Gemma chat templating, masks control/modality tokens, and trains only on the assistant JSON completion.
...
- Example configs for Qwen2.5‑VL, Gemma 4, and SmolVLM2 LoRA fine‑tuning.
...
- Gemma 4 support requires `transformers>=5.5.3`.
- SmolVLM2 support requires `transformers>=5.5.3` and `num2words`.
